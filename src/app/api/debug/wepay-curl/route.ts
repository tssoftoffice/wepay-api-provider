...
export async function GET() {
    try {
        ...
        // Note: Using -k (insecure) to rule out SSL verification issues on client side locally
        // Using -F to send multipart/form-data
        const cmd = `curl -X POST ${WEPAY_ENDPOINT} -F "username=${USERNAME}" -F "password=${PASSWORD}" -F "type=balance_inquiry" -v`
